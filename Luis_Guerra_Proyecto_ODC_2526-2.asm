...
#     * Binario (Complemento a 2, 8-bit entera / 8-bit fraccionaria).
#     * Decimal Empaquetado (BCD con nibbles de signo C/D).
#     * Octal y Hexadecimal mediante algoritmos de divisiones sucesivas.
# - Robustez: Validación de entrada de strings para prevenir desbordamientos y errores 
...
.macro imprimir_hex_fraccionario(%reg_valor_total, %reg_num_decimales)
    ...
    bltz %reg_valor_total, hex_f_neg
    li $a0, '+'
    li $v0, 11
    syscall
    j hex_f_pos
...
.macro imprimir_octal_fraccionario(%reg_valor_total, %reg_num_decimales)
    ...
    bltz %reg_valor_total, oct_f_neg
    li $a0, '+'
    li $v0, 11
    syscall
    j oct_f_pos
oct_f_neg:
    ...
    syscall
oct_f_pos:
    # 3. Imprimir parte entera en Octal
    move $a0, $t1
...
oct_ent_neg:
    ...
    li $v0, 11
    syscall
    neg $t0, $t0          # Valor absoluto
es_pos_oct:
    # 2. Conversión a Octal (reversa)                
    la $t1, buffer_bcd          # buffer temporal para guardar los dígitos
...
leer_entero:
    ...
    # Validar lo que el usuario escribio en 'buffer'
...
leer_fraccionario:
    ...
    # Si $s1 es 1, error. Volver a pedir el número fraccionario
